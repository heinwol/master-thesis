...
#proposition[
  ...
  $
    (limits(sect.big)_(v in tilde(V)_1) Stab_(H_1)(v)) times (limits(sect.big)_(v in tilde(V)_2) Stab_(H_2)(v)) <= Aut(G) <= H_1 times H_2,
  $
...
#proposition[
  Пусть дана произвольная динамическая система $cal(S) = (X, S)$, $Aut(cal(S))$ -- группа ее автоморфизмов, $x in X$ -- произвольное состояние, а $Stab(x)$ -- стабилизатор состояния $x$ относительно действия группы $Aut(cal(S))$ на $X$, т.е. $Stab(x) = {Phi in Aut(cal(S)) | Phi(x) = x}$. Тогда $forall n in NN sp Stab(S^n (x)) subset Stab(x)$. Иначе говоря, траектория, соответствующая начальному состоянию $x$ является целиком "самосимметричной", если начальное состояние было "самосимметричным".
] <prop:sym_traj>
...
#remark[
  ...
  // #figure(
  //   caption: [Ресурсная сеть, функция эволюции которой не ялвяется инъективной.],
