#import "./template.typ": thmrules, template, definition, theorem, proof, proposition, code, lemma, corollary
...
#proof[
  ...
  $
    sigma(F(q)) = sigma(min{R' dot.circle (bold(1) dot q), R}) = min{sigma(R') dot.circle (bold(1) dot sigma(q)), sigma(R)}.
  $ <nonum>
  ...
  $
    sigma(F(q)) = min{sigma(R') dot.circle (bold(1) dot sigma(q)), sigma(R)} = min{R' dot.circle (bold(1) dot sigma(q)), R} = F(sigma(q)).
  $ <nonum>
...
Обозначим группу автоморфизмов ресурсной сети $"RN"$ как графа $"Aut"_"graph" ("RN")$, а группу автоморфизмов ее как динамической системы -- $"Aut"_"dyn" ("RN")$.

#corollary[
  Для произвольной ресурсной сети $"RN"$ верно

  $ "Aut"_"graph" ("RN") <= "Aut"_"dyn" ("RN"). $ <nonum>
]

Заметим, что из вышесказанного еще не следует, что симметрии ресурсной сети как динамической системы ограничиваются симметриями соответствующего графа.
