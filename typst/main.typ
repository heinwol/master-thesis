...
#definition[
  Пусть дано произвольное множество $X$ и некоторая функция $S: X -> X$. *Дискретная динамическая система* может быть образована из $S$ следующим образом: если $q(0) in X$ -- некотрое начальное состояние, то $q(t) = S(q(t-1)), t in NN$. $S$ при этом называется *функцией эволюции динамической системы*.
]

#definition[
  *Автоморфизм дискретной динамической системы* -- биекция $Phi : X -> X$, согласованная с действием динамической системы, а именно

  $
    forall x in X sp S(Phi(x)) = Phi(S(x)).
  $ <nonum>
]
