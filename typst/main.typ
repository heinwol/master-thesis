...
#proposition[
  Предположим, что, в нарушение базовых предположений о губковой сети, веса "сверху вниз" и "снизу вверх" в сети $"SN"$ совпадают. Предположим также, что в сети нет стоковых вершин. Пусть сеть треугольная или прямоугольная и, дополнительно, в случае треугольной сети количество строк четно ($n = 2k$), а в случае прямоугольной $m != n$. В данном случае сеть будет иметь еще один автоморфизм $Phi: Phi((i, j)) = (i, 2 ceil(n "/" 2) - j)$ и, соответственно, $"Aut"("SN") tilde.equiv quo(ZZ, 2 ZZ)$ в случае треугольной сети и $"Aut"("SN") tilde.equiv (quo(ZZ, 2 ZZ))^2 = K_4$ (четвертая группа Клейна, полная группа симметрий прямоугольника) в случае прямоугольной сети.
]
...
Пусть дана некоторая сеть $"SN"$, удовлетворяющая базовым предположениям о губковой сети. Пусть $tilde(m) = m$, если сеть прямоугольная или шестиугольная и $tilde(m) = floor(m "/" 2)$, если сеть треугольная. Дополнительно, пусть $m$ -- четно, если сеть треугольная и шестиугольная. Рассмотрим следующее отношение эквивалентности $~_("cyl")$ на множестве вершин сети $V$:
$
  u sp ~_("cyl") v <=> cases(delim: "[",
    u = v\,,
    j_1 = j_2 and (i_1\, i_2 in {0, tilde(m)})\, #[где] (i_1, j_1) = u\, (i_2, j_2) = v.
  )
$

Будем называть $quo("SN", ~_("cyl"))$ *губковой сетью на цилиндре*. Приведенную факторизацию можно рассматривать как помещение губковой сети на цилиндр: левый и правый края сети "склеиваются" в одну линию. Пример такой сети приведен на @fig:sn_on_cylinder_1.
...
#lemma[
  Губковая сеть $quo("SN", ~_("cyl"))$ факторизована идеально.
] <lem:ideal_cylinder>
...
При этом, как следует из леммы @lem:ideal_cylinder, веса ребер в сети не меняются. Если сеть прямоугольная, или же если она треугольная или шестиугольная, но число столбцов в ней четно, то будем говорить, что такая сеть *может быть помещена на цилиндр*.
...
#theorem[
  Пусть $"SN"$ -- некоторая губковая сеть, которая может быть помещена на цилиндр, а $"Aut"("SN")$ -- ее группа симметрий. Тогда

  $
    "Aut"(quo("SN", ~_("cyl"))) tilde.equiv "Aut"("SN") times quo(ZZ, (tilde(m) - 1) ZZ).
  $
]

Дополнительно был доказан следующий факт: dd ds ds dsadsa
