...
#definition[
  *Замощением плоскости* называется разбиение плоскости на множество многоугольников $cal(F)$, при этом "типов" многоугольников -- конечное число, в том смысле что множество, состоящее из орбит всех элементов множества $cal(F)$ под действием группы $"Sym"(cal(F))$ -- иначе говоря, $cal(F) "/" "Sym"(cal(F))$ -- конечно.
]

#definition[
  Замощение плоскости называется *регулярным*, если группа $"Sym"(cal(F))$ действует на $cal(F)$ транзитивно, т.е. $|cal(F) "/" "Sym"(cal(F))| = 1$.
]
...
#theorem[
  ...
  Верно следующее:
  - $"SN"$ обладает осевой симметрией, т.е. $"Aut"("SN") tilde.equiv ZZ "/" 2 ZZ$, если сеть:
    - прямоугольная;
    - шестиугольная, при этом $m$ -- нечетное;
    - треугольная, при этом $m$ -- нечетное и $m > 1$;
  - $"Aut"("SN") tilde.equiv limits(times.big)_(i = 1)^(ceil(n "/" 2)) ZZ "/" 2 ZZ$ если сеть треугольная и $m = 1$;
...
#proposition[
  Предположим, что, в нарушение базовых предположений о губковой сети, веса "сверху вниз" и "снизу вверх" в сети $"SN"$ совпадают. Предположим также, что в сети нет стоковых вершин. Пусть сеть треугольная или прямоугольная и, дополнительно, в случае треугольной сети количество строк четно ($n = 2k$), а в случае прямоугольной $m != n$. В данном случае сеть будет иметь еще один автоморфизм $Phi: Phi((i, j)) = (i, 2 ceil(n "/" 2) - j)$ и, соответственно, $"Aut"("SN") tilde.equiv ZZ "/" 2 ZZ$ в случае треугольной сети и $"Aut"("SN") tilde.equiv (ZZ "/" 2 ZZ)^2 = K_4$ (четвертая группа Клейна, полная группа симметрий прямоугольника) в случае прямоугольной сети.
]
...
#definition[
  Пусть $G = (V, E)$ -- некоторый ориентированный граф. Допустим, на множестве вершин $V$ задано произвольное отношение эквивалентности "$~$". Образуем *факторграф графа $G$ по отношению $~$* следующим образом @Gardiner1974. $G "/" ~ eq.def (V', E'):$
...
Как можно видеть, так же, как и в случае обычного факторграфа, в при факторизации в ресурсной сети не появляется никаких "новых" ребер в том смысле что количество ребер в факторизованной сети не больше, чем в исходной. Отметим также, что данное определение не согласуется с определением обычного ориентированного факторграфа. Действительно, если представить себе некоторый граф $G$ без петель, в котором отождествлены смежные вершины $u$ и $v$, то в $G "/" ~$ появится петля $(esq(u), esq(u)) = (esq(v), esq(v))$. Напротив, если представить себе подобную ситуацию с ресурсной сетью $"RN"$, то новых петель в ходе факторизации не появится. Такая разница введена для того, чтобы различить разные по своей сути классы ребер в ресурсной сети с жадными вершинами: петли и все остальные ребра. Действительно, если бы мы разрешили отождествлять вершины без петель так, чтобы в результате получалась "жадная" вершина, это бы заметно изменило качественные свойства сети. Впрочем, мы будем рассматривать несколько иные ситуации, подпадающие под определение @def:ideal-quotient.

#definition[
  Пусть $"RN"$ -- произвольная ресурсная сеть (возможно, с жадными вершинами). Отношение эквивалентности на множестве вершин сети "$~$" назовем *идеальным* (и, соответственно, получившуюся фактор-сеть $"RN""/" ~$ назовем *идеально факторизованной*), если $forall esq(u) in V "/" ~$, $forall esq(v) in V "/" ~$ выполняются следующие условия:
...
#proposition[
  Пусть $J = {esq(v) | esq(v) in V "/" ~ and abs(esq(v)) > 1 }$. Тогда если $forall esq(u) in J, forall esq(v) in J : esq(u) != esq(v) sp (forall u' in esq(u), forall v' in esq(v) sp "adj"_(G)(u') sect "adj"_(G)(v') = emptyset)$, то выполнено второе условие из определения @def:quotient-network. Если, дополнительно, веса петель в каждом классе эквивалентности равны, то $"RN""/"  ~$ факторизуется идеально.
] <prop:simple_ideal>
