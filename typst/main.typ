...
Будем теперь иметь в виду, что вершины в губковых сетях допускают нумерацию парой натуральных чисел, как демонстрировалось на приведенных выше рисунках. При этом вершина с наименьшими индексами $(x, y)$ находится "слева снизу", а с наибольшими -- "справа сверху". При этом стоковые вершины нумеруются индексами $(x, -1)$.
...
Пусть $m$ -- количество строк в губковой сети, а $n$ -- количество столбцов. Будем исходить из того, что веса ребер в сети таковы, что:
...
// $ 
// forall v_1 = (x_1, y_1) in V, forall v_2 = (x_2, y_2) in V, forall v_3 = (x_3, y_3) in V, forall v_4 = (x_4, y_4) in V :\

// (v_1, v_2) in E, (v_3, v_4) in E "выполняется следующее:"\

// cases(
//   phi((v_1, v_1)) = phi((v_2, v_2)),
//   (v_1, v_2) in E and y_1 < y_2 => 
// )
// $
