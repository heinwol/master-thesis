...
В качестве примера, когда стабилизатор "расширяется" под действием $S$, приведем следующую динамическую систему $cal(S)$. Пусть $X$ -- множество конечных последовательностей из натуральных чисел. Например, $(1, 4, 2) in X$ или же $(3, 3, 7, 4, 5) in X$. Пусть $S$ "забывает" последний элемент последовательности, то есть $S : (x_1, x_2, ..., x_(n-1), x_n) arrow.bar (x_1, x_2, ..., x_(n-1))$, а $S((x_1)) = (x_1)$. Можно показать, что любой автоморфизм данной динамической системы имеет вид $Phi : (x_1, x_2, ..., x_n) |-> (f_1 (x_1), f_2 (x_2), ..., f_n (x_n))$, где каждая из функций ${f_i}_(i = 1)^infinity$ -- биекция.
