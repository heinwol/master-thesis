...
=== Симмертичность и асиммертичность губковых сетей

Как можно видеть, губковые сети -- в отличие от ресурсных сетей вообще -- имеют довольно симметричную структуру. Попробуем определить более формально, что это означает. Для начала, вслед за Грюнбаумом @Gruenbaum1977, определим тот вид симметрии, о котором идет речь.

#definition[
  Пусть $cal(F)$ -- некоторое разбиение плоскости на измеримые множества ненулевой меры. Назовем $"Sym"(cal(F)) eq.def ("Trans"(RR^2) dot O(2)) sect {phi in (RR^2 -> RR^2) | forall d in cal(F) sp phi(d) in cal(F)}$ *группой симметрий этого разбиения*, где $"Trans"(RR^2) dot O(2)$ -- группа движений евклидовой плоскости @Винберг2019[стр. 164]. Иначе говоря, это такой набор движений плоскости (вращений и параллельных переносов), который переводит все множества из $cal(F)$ в (возможно, другие) множества из $cal(F)$. Очевидно, что $"Sym"(cal(F))$ -- группа.
]
...
Пусть $G$ -- бесконечный локально конечный планарный граф (т.е. $|V| = infinity$, но $forall v in V |"adj"(v)| < infinity$, где $"adj"(v)$ -- множество вершин, смежных с $v$), а $"emb"(G)$ -- вложение этого графа в плоскость. Скажем, что $G$ определяет разбиение плоскости следующим образом: рассмотрим множество $RR^2 \\ "emb"(G)$. Скажем, что $cal(F)$ есть множество связных компонент $RR^2 \\ "emb"(G)$ (либо же замыканий этих связных компонент, если нам удобно рассматривать множества с границей). Без ограничения общности, можно считать, что $cal(F)$ состоит из многоугольников.

Очевидно, что конкретное разбиение будет зависеть от выбора вложения $G$. Верно также и то, что среди всех вложений $"Emb" = {"emb" | "emb" - #[вложение графа $G$ в плоскость]}$ существуют *максимальные* по насыщенности групповой структуры получившегося разбиения, т.е.

$
  exists e in "Emb" : forall e' in "Emb" sp "Sym"(cal(F)(e'(G))) <= "Sym"(cal(F)(e(G))),
$
где $cal(F)("emb"(G))$ -- разбиение, соответствующее вложению $"emb"$, а группы $H_1$ и $H_2$ связаны отношением $H_1 <= H_2$ если $H_1$ изоморфна некоторой подгруппе $H_2$. Не задаваясь вопросом о единственности такого вложения $e$, скажем, что не составит труда привести такие графы и их вложения, чтобы соответствующие им разбиения были треугольным, прямоугольным или шестиугольным замощением плоскости; при этом такие вложения будут максимальными для соответствующих графов. Назовем эти графы и вложения *каноническими*.
