...
Пример ресурсной сети $"nw"_1$ можно видеть на @fig:basic_network_1. Множество вершин здесь есть $"nw"_1.G.V = {0, 1, 2}$, а метки ребер обозначают соответствующие веса в графе $"nw"_1.G$. На @fig:basic_network_2 показано некоторое состояние сети с ресурсами #state_dict(state_1). При данном способе визуализации вершины имеют разный размер в зависимости от количества имеющегося в них ресурса. Более того, вершины $v_i$, в которых ресурс не меньше порогового значения (т.е. $q_i >= r_i^("out")$), окрашены в розоватый цвет, а остальные -- в салатовый. 
...
#grid(
  ...
  [
    #figure(
      ...
      generated_image("basic_network/plot.svg")()
    ) <fig:basic_network_1>
  ],
  [
    #figure(
      caption: [Состояние сети $"nw"_1$ при #state_dict(state_1).],
      generated_image("basic_network/sim.svg")()
    ) <fig:basic_network_2>
  ],
)

Матрица пропускной способности $"nw"_1$ приведена в уравнении @eq:basic_network_R, а ее стохастическая матрица -- в уравнении @eq:basic_network_R_. Можно убедиться, что сумма значений в каждой строке стохастической матрицы равна единице.
