...
$
r_i^(i n) = sum_(j = 1)^n r_(j i) и r_i^("out") = sum_(j = 1)^n r_(i j)
$ <nonum>
...
$ sum_(j = 1)^n f_(i j) (t) = sum_(j = 1)^n r_(i j) = r_i^("out") . $
...
Рассмотрим ресурсную сеть с двумя сильно связанными компонентами, т.е. такую сеть, в которой есть только ребра от первой сильно связной компоненты (называемой #emph[переходной] компонентой с $n_1$ вершинами) до второй (называемой #emph[финальной];, с $n_2$ вершин). Пусть n — общее количество вершин в сети (Рис. 1).

Рисунок 1.1 — Двухкомпонентная ресурсная сеть. Вершины ($v_1$ - $v_4$) принадлежат переходной составляющей; вершины ($v_5$ - $v_9$) образуют финальную компоненту;

$n_1 = 4 , n_2 = 5 , n = 9$.

#theorem[
  Существует конечный момент времени $t'$ такой, что при $t > t'$ все вершины переходной компоненты работают по правилу 2.
]
...
$
cases(
  r_(i j) (t) = r_(i i) + (q_i (t) - r_i^("out"))"," & upright("если ") q_i (t) > r_i^("out")  и  i = j,
  r_(i j) "," &  "иначе"
)
$ <nonum>

Здесь, $forall t gt.eq 0    Q (t + 1) = Q (t) R^prime (t)$, или
...
$
Q^(\*) = Q (0) lim_(t arrow.r oo) product_(i = 0)^t R^prime (i) = Q (
  0
) R^(prime oo)
$

где $ R^prime^oo  =  bold(1)  bullet  Q^(1 \* \*)  \(  Q^(1 \* \*)$ — некоторый вектор респределения ресурса $ W = 1 $).

Следующая теорема устанавливает связь модели двухкомпонентной ресурсной сети с неоднородными цепями Маркова в явном виде:

#theorem[ Для двухкомпонентной ресурсной сети, если $W  >  T$, то для любого начального состояния неоднородная цепь Маркова со стохастическими матрицами $R^prime (t)$ сильно эргодична].
