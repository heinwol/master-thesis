#import "./template.typ": thmrules, template, definition, theorem, proof, proposition, code, lemma
...
#definition[
  Пусть дана некоторая губковая сеть $"SN"$. Отображение $Phi : "SN" -> "SN"$ будем называть *автоморфизмом губковой сети*, если оно является автоморфизмом графа $G$, соответствующего этой сети, которое сохраняет, при этом, веса ребер, т.е. $forall e in "SN".G.E sp w(e) = w(Phi(e))$, где $w(e)$ -- вес ребра $e$, а также переводит верхние вершины в верхние.

  *Группой автоморфизмов губковой сети $"SN"$* назовем множество всех автоморфизмов данной сети $"Aut"("SN")$, в котором композиция функций выступает в качестве бинарной операции, наделяя указанное множество групповой структурой.
] <def:sn_aut>
...
#proof[
  ...
  $
    Phi|_("Gen"_(G)(V_(i+1) union V_i)) in {id, sp id_(V_(i+1)) union.sq S_i, sp S_(i+1) union.sq id_(V_i), sp S_(i+1) union.sq S_(i)}.
  $ <nonum>
...
#proposition[
  ...
  $
    (limits(sect.big)_(v in tilde(V)_1) "Stab"_(H_1)(v)) times (limits(sect.big)_(v in tilde(V)_2) "Stab"_(H_2)(v)) <= "Aut"(G) <= H_1 times H_2,
  $
  где $"Stab"_H (x)$ -- стабилизатор элемента $x$ при действии группы $H$ на него.
]
...
#definition[
  Зададимся некоторым (взвешенным, ориентированным) графом $G$. Примем следующие обозначения: для произвольной вершины $v in V$ $"in"(v) = {u in V | (u, v) in E}$, $"out"(v) = {u in V | (v, u) in E}$, а $Sigma$ -- множество (всевозможных) перестановок. Пусть $F$ -- некоторое множество. Образуем *обобщенную систему распределения на графе* следующим образом. Каждой вершине $v in V$ графа припишем некоторое состояние $q_v in F$; множество всех состояний тогда $Q = F^V$. Функция эволюции динамической системы $S : Q -> Q$ должна удовлетворять следующим свойствам:

  + _Локальность_. Состояние вершины  $v$ в следующий момент времени зависит только от ее состояния; весов ребер, которые ведут из этой вершины; состояния вершин, из которых ведут ребра в эту вершину (т.е. $"in"(v)$) и весов этих ребер;
  + _Коммутативность_. Состояние вершины $v$ не зависит от порядка вершин в $"in"(v)$.

  Формально определим это так. Пусть $v$ и $v'$ -- произвольные вершины, для которых $exists sigma_1 in Sigma : "in"(v) limits(->)^(sigma_1) "in"(v')$, $exists sigma_2 in Sigma : "out"(v) limits(->)^(sigma_2) "out"(v')$ такие, что $(forall u in "in"(v) sp w((u, v)) = w((sigma_1(u), v'))) and (forall u in "out"(v) sp w((v, u)) = w((v', sigma_2(u))))$. Пусть $q$ -- произвольное состояние системы, а $q' = S(q)$. Тогда необходимо выполняется $(q_v = q_(v') and forall u in "in"(v) sp q_u = q_(sigma_1(u))) => q'_v = q'_(v')$.
  // При этом указанное условие должно выполняться для всех возможных перестановок $sigma_1$.
] <def:GSD>

Конечно, приведенное обобщение ресурсной сети весьма абстрактно. В нашем обсуждении важно лишь одно свойство -- свойство симметричности поведения динамической системы -- которое и было выделено в определении @def:GSD.

Можно проверить, что ресурсная сеть (обыкновенная, с жадными вершинами, губковая сеть) вполне удовлетворяет этому определению. Действительно, из уравнения функции эволюции ресурсной сети @eq:S выводится, что состояние $q_i(t+1)$ зависит только от $q_i(t)$, $i$-й строки и $i$-го столбца матрицы потока. Далее, если предположить, что вершины $v_i$ и $v_j$ имеют одинаковый набор ребер, смежных с ними, то строки и столбцы матрицы пропускной способности $R$ под номерами $i$ и $j$ должны совпасть, по крайней мере, с точностью до некоторой перестановки. Поскольку совпадают строки $i$ и $j$ матрицы $R$, то совпадают и строки стохастической матрицы $R'$, поскольку строка $R'$ и строка $R$ отличаются друг от друга лишь на коэффициент $r_i^"out"$, который есть сумма элементов в данной строке. Затем, если совпадают $i$ и $j$ строки у $R$ и у $R'$, то из определения @def:flow-matrix видно, что суммы элементов в $i$ строке и в $j$ строке также совпадают, если $q_i = q_j$.

Аналогичные по своей сути, но более простые по форме рассуждения показывают, то определение @def:GSD применимо также и к модели цепи Маркова, где состоянием является распределение вероятностей нахождения объекта в вершине цепи.
