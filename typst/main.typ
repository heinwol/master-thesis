...
#proof[
  #fli[
    ...
    Однако же, если сеть треугольная и в ней присутствует всего один столбец, то наше рассуждение неверно, так как один уровень ($V_(i+1)$ или $V_i$) состоит из единственной вершины (и тогда, конечно, либо $S_(i+1) = id$, либо $S_i = id$) и можно проверить, что тогда $Phi$ действительно автоморфизм на $"Gen"_(G)(V_(i+1) union V_i)$.
...
#claim[
  Если $Phi in Aut(G)$, а $V' in V$ -- инвариантное относительно $Phi$ подмножество вершин графа (т.е. $Phi(V') = V'$), то \ $Phi|_V' in Aut("Gen"_G (V'))$.
]
...
#proof[
  Любой автоморфизм $Phi$ группы $G$, как было сказано, переводит вершины из $V_i$ в $V_i$. В таком случае, #comment[Дописать]
]
...
#proof[#fli[
    ...
    $
      sigma(F(q)) = min{sigma(R') dot.circle (bold(1) dot sigma(q)), sigma(R)} = \
...
#[
  ...
  Приведем теперь пример такой сети $"SN"$ и такого состояния $q^0$, что $Stab(q^0) != {id}$ (@fig:sponge_symmetrical_sim). Здесь
